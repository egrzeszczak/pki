...
\begin{tabular}{| m{3cm} | m{7cm}| m{7cm} |}
    ...
    \hline
    Key aspects & Symmetric cryptography uses \textbf{one key} to encrypt and decrypt information. & Asymmetric cryptography uses \textbf{two keys} that are mathematically-bound, where one is public and shared with others and the other one is kept private. Public key is used for encrypting messages, and the private key is used to decrypting messages. \\
    \hline
    Key security & Anyone that has access to the key can decrypt the ciphertext. & You can only decrypt information if you are in possesion of the private key. Having access to the public key does not give the adversary the ability to decrypt information. \\
    \hline
    ...
    \hline
    Assurances & Symmetric ciphers provide only \textbf{Confidentiality}. & Asymmetric ciphers provide \textbf{Confidentiality}, as well as \textbf{Integrity} and \textbf{Non-repudiation}.\\
    \hline
\end{tabular}
}
\end{center}
